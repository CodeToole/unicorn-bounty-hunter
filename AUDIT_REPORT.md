...
* **Description:**
  * The default fallback secret key `[REDACTED_ADMIN_SECRET]` is hardcoded in `config.py` when `ADMIN_SECRET_KEY` / `ADMIN_KEY` environment variables are not supplied.
...
* **Description:**
  ...
  * The admin key is passed in URL query strings across navigation links (e.g. `<a href="/admin?key=[REDACTED_ADMIN_SECRET]&tab=posts">`).
